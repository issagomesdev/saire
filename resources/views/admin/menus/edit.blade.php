    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.menus.update", [$menu->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Menu::LINK_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('link_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="link_type_{{ $key }}" name="link_type" value="{{ $key }}" {{ old('link_type', $menu->link_type) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group" id="submenu" style="display: none">
                ...
                <select class="form-control select2 {{ $errors->has('submenuses') ? 'is-invalid' : '' }}" name="submenuses[]" id="submenuses" multiple disabled>
                    @foreach($submenuses as $id => $submenus)
                        <option value="{{ $id }}" {{ (in_array($id, old('submenuses', [])) || $menu->submenuses->contains($id)) ? 'selected' : '' }}>{{ $submenus }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" id="url" style="display: none">
                ...
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $menu->url) }}" disabled>
                @if($errors->has('url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('url') }}
                    </div>
